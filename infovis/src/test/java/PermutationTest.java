...
public class PermutationTest extends TestCase {
    ...
    public void testPermutation() {
        ...
        assertEquals(10, perm.getMaxIndex()+1);
        for (int i = 0; i < 9; i++) {
            if (i < 4) {
                assertEquals(9-i, perm.getDirect(i));
            }
        ...
        int[] perm2 = { 8, 2, 4, 7 };
        perm.permute(perm2);
        assertEquals(perm2.length, perm.size());
        assertEquals(0, perm.getDirect(0));
        assertEquals(7, perm.getDirect(1));
        assertEquals(4, perm.getDirect(2));
        assertEquals(1, perm.getDirect(3));
        //TODO test moveTo
    }
