 ...
 */
public class AdditiveAggregation extends AbstractMetadata 
    ...
    public boolean isAggregating(Column col, Tree tree) {
        String aggr = (String)getValue(col, AGGREGATION_TYPE);
        ...
        if (ret == AGGREGATE_YES) {
            setValue(col, AGGREGATION_TYPE, AGGREGATION_TYPE_ADDITIVE);
            return true;
        }
        return false;
    }
    ...
    public Column aggregate(Column src, Tree tree, Column dst) {
        ...
        buildAdditiveWeight(
                ...
                (NumberColumn) dst);
        setValue(dst, AGGREGATION_TYPE, getType());
        return dst;
    }
