...
public class FRLayout extends BasicSpringLayout {
    ...
    protected void initializeLayout(Rectangle2D bounds) {
        ...
        forceConstant = 
            ...
            / getGraphVisualization().getVerticesCount());
    }
    ...
    protected double repulsiveForce(double length) {
        return (forceConstant * forceConstant) / length; 
    }
    
    protected double attractiveForce(double length) {
    ...
    protected void calcRepulsion(int v) {
        ...
        for (RowIterator iter2 = getGraphVisualization().vertexIterator(); iter2.hasNext();) {
            int u = iter2.nextRow();
