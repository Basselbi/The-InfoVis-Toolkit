...
public class BreadthFirst extends Algorithm {
    ...
    /**
     * <b>Visitor</b> is used to iterate over all the vertices.
     * 
     * @author Jean-Daniel Fekete
     */
    ...
    /**
     * <b>Manager</b> is a convenient class implementing <code>Visitor</code>.
     * 
     * @author Jean-Daniel Fekete
     */
    public static class Manager implements Visitor {
        /**
         * {@inheritDoc}
         */
        public void blackTarget(int edge) {
     ...
     * @param color a hash map of visited vertices or null
     * @param edgeIter an allocated iterator or null
     */
    public void visit(
            Visitor vis, 
            int start, 
            OpenIntIntHashMap color, 
            RowIterator edgeIter) {
        if (color == null) {
            color = new OpenIntIntHashMap();
        }
        else {
            color.clear();
        }
        if (edgeIter == null) {
            edgeIter = graph.newEdgeIterator();
        }
        IntLinkedList Q = new IntLinkedList();

        color.put(start, GREY);
        Q.addLast(start);
        setProgressValues(0, graph.getVerticesCount(), "Breath First Search");
        int count = 0;
        while (!Q.isEmpty()) {
            int u = Q.getFirst();
            Q.removeFirst();
            ...
            for (RowIterator iter = graph.isDirected() 
                    ? graph.outEdgeIterator(u, edgeIter) 
                    : graph.edgeIterator(u, edgeIter);
                iter.hasNext();) {
                int e = iter.nextRow();
