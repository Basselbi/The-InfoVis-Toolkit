...
import infovis.column.format.FormatMetadata;
 ...
 * @infovis.factory GraphReaderFactory graphml
 */
public class GraphMLReader extends AbstractXMLReader {
    ...
    protected FormatMetadata format;
    protected String       ID;
    protected Map          idName;
    ...
    private static Logger LOG = Logger.getLogger(GraphMLReader.class);
     ...
     * 
     * @param keyFor
     *            category the key is for, can be graph, edge or node.
     * @param ID
     *            identificator for this key.
     * @param type
    ...
    public void declareKey(String keyFor, String ID, String type) {
        ...
        if (keyFor.equals("node")) {
            ...
            if (c == null) {
                ...
                if (c.getFormat() instanceof FormatMetadata) {
                    format = (FormatMetadata) c.getFormat();
                }
                else {
                    format = null;
                }
            ...
            Column c = edgeTable.getColumn(name);
            if (c == null) {
                c = createColumn(type, name);
    ...
    public void startElement(
        ...
        if (firstTag) {
            if (!qName.equals("graphml"))
                throw new WrongFormatException(
                        "Expected a graph toplevel element");
            firstTag = false;
        }
        ...
        if (qName.equals("key")) {
            ...
            if (format == null) {
                LOG.warn("type parameters specified for non-parametric type "+type);
            }
            else {
                format.addMetadata(t, name, value);
            }
        }
