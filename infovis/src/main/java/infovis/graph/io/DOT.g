...

{
	...
	/** Name of the column containing dot labels. */
	public static final String DOT_LABEL_COLUMN = "label";
	...
	String attributePrefix = "dot::";
	String nodeAttributePrefix = "dot_node::";
	String edgeAttributePrefix= "dot_edge::";
    //Column labels;
	...
	Rectangle2D.Float parseBbox(String value) {
		...
		float xmin = Float.parseFloat(values[0]);
		float ymin = Float.parseFloat(values[1]);
		float xmax = Float.parseFloat(values[2]);
		float ymax = Float.parseFloat(values[3]);
	...
	void addGraphAttributes(ArrayList attrs) {
		for (int j = 0; j < attrs.size(); j += 2) {
			...
			graph.getMetadata().addAttribute(metaName, value);
			if (name.equals("bb")) {
	...
	protected int parsePoint(String pos, int index, Point2D p) {
		while(pos.charAt(index)==' ') {
        	...
        	int i = pos.indexOf(',', index+1);
                if (i == -1)
                    return -1;
                float x = Float.parseFloat(pos.substring(index, i));
                float y;
                index = pos.indexOf(' ', i+1);
                if (index == -1)
                	y = Float.parseFloat(pos.substring(i+1));
                else 
                	y = Float.parseFloat(pos.substring(i+1, index));
                p.setLocation(x, y); 
                return index;
	}
	
	void parseEdgeValue(int e, String pos) {
            Shape s = graphReader.getLinkShape(e);
            GeneralPath p = null;
            if (s instanceof GeneralPath) {
                ...
                if (pos.charAt(index)=='e' ||
                    ...
                    arrow_x = (float)pt.getX();
                    arrow_y = (float)pt.getY();
                }
			...
			for (int j = 0; j < attrs.size(); j += 2) {
				...
				if (name.equals(DOT_POS_COLUMN)) {
    				...
    				if (c == null) {
    					...
    					String propName = edgeAttributePrefix+name;
    					graph.getMetadata().addAttribute(propName, name);
    				}
		...
		for (int i = 0; i < nl.size(); i++) {
			...
			for (int j = 0; j < attrs.size(); j += 2) {
				...
				if (name.equals(DOT_POS_COLUMN)) {
					...
					s.x = Float.parseFloat(value.substring(0, index));
					s.y = Float.parseFloat(value.substring(index+1));
				}
				else if (name.equals(DOT_WIDTH_COLUMN)) {
					Rectangle2D.Float s = (Rectangle2D.Float)graphReader.findNodeShape(v);
					s.width = 72*Float.parseFloat(value);
					s.x -= s.width/2;
				}
				else if (name.equals(DOT_HEIGHT_COLUMN)) {
					Rectangle2D.Float s = (Rectangle2D.Float)graphReader.findNodeShape(v);
					s.height = 72*Float.parseFloat(value);
					s.y -= s.height/2;
				}
				/*
				else if (name.equals(DOT_LABEL_COLUMN)) {
					if (labels != null) {
						labels.setValueOrNullAt(v, value);
					}
				}
				*/
				else {
    				...
    				if (c == null) {
    					...
    					String propName = nodeAttributePrefix+name;
    					graph.getMetadata().addAttribute(propName, name);
    				}
