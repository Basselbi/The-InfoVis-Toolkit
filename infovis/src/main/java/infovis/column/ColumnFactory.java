...
public class ColumnFactory extends BasicFactory {
    ...
    public void add(String type, Creator c) {
        ...
        ArrayList cl = (ArrayList)getEntry(type);
        if (cl == null) {
            cl = new ArrayList();
            putEntry(type, cl); 
        }
    ...
    public void add(String type, String columnClass, String density) {
        ...
        if (density.equals("DENSE")) {
            ...
            LOG.error("Bad density name in ColumnFactory"+density);
            throw new RuntimeException("Bad density name in ColumnFactory"+density);
        }
        add(type, new DefaultCreator(type, densityType, columnClass ));
        add(columnClass, new DefaultCreator(type, densityType, columnClass ));
    }
    ...
    public boolean remove(Creator c) {
        ArrayList cl = (ArrayList)getEntry(c.getTypeName());
    ...
    public Creator getCreator(String typeName, int type) {
        ...
        ArrayList cl = (ArrayList)getEntry(typeName);
        if (cl == null) {
            return null;
        }
    ...
    public Column create(String typeName, int type, String name) {
        ...
        if (i != -1) {
            if (typeName.endsWith("]")) {
                params = typeName.substring(i+1, typeName.length()-1);
            }
    ...
    public interface Creator {
         ...
         * Create a column with a specified name. 
         * @param name the column name.
         * @param params supplemental parameters 
         * @return a column with a specified name. 
         * */
        Column create(String name, String params);
    }
    ...
    public static class DefaultCreator implements Creator {
        ...
        public Column create(String name) {
            if (columnClass == null) {
                try {
                    ...
                    LOG.error("Cannot load class named "+className);
                    return null;
                }
            }
            try {
                ...
                LOG.error("Cannot instantiate Column class "+className, e);
            }
            return null;
        }
         ...
         * {@inheritDoc} 
         */
        public Column create(String name, String params) {
            if (columnClass == null) {
                try {
                    columnClass = Class.forName(className);
                }
