...
/**
 * <b>ChunkedDoubleColumn</b> is a double column
 * allocating values by chunks.
 * 
 * @author Jean-Daniel Fekete
 * @version $Revision: 1.2 $
 */
public class ChunkedDoubleColumn extends AbstractDoubleColumn {

    protected transient ArrayList     chunks;
    protected int           size;
    /** Number of bits per chunk. */
    public static final int CHUNK_BITS = ByteBufferPool.CHUNK_BITS/2;
    /** Size of allocated chunks. */
    public static final int CHUNK_SIZE = ByteBufferPool.CHUNK_SIZE/2;
    /** Mask of bits. */
    public static final int CHUNK_MASK = ByteBufferPool.CHUNK_MASK>>1;
     ...
     * @param name the name
     */
    public ChunkedDoubleColumn(String name) {
     ...
     */
    public ChunkedDoubleColumn(String name, int reserve, ByteBufferPool pool) {
        super(name);
        this.pool = pool;
    ...
    protected void newChunk(int c) {
        ByteBuffer chunk = ByteBuffer.allocate(8*CHUNK_SIZE);
        chunks.set(c, chunk);
    }
    ...
    public double get(int index) {
        ...
        return getChunk(index).getDouble(8*i);
    }
    ...
    public void set(int index, double value) {
        int i = index & CHUNK_MASK; // indexToCIndex(index);
        getChunk(index).putDouble(8*i, value);
        set(index);
    }
    ...
    private void writeObject(ObjectOutputStream s) 
        ...
        int n = chunks.size();
        s.writeInt(n);
        byte[] buffer = new byte[CHUNK_SIZE*8];
        for (int i = 0; i < n; i++) {
