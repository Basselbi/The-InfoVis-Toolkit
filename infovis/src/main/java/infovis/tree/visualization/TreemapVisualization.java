...
//        getVisualColumnDescriptor(VISUAL_SIZE).setFilter(filter);
    ...
    public Item pickTop(Rectangle2D hitBox, Rectangle2D bounds) {
        ...
        return pickTop(0, hitBox, bounds); 
    }

    public Item pickTop(int parent, Rectangle2D hitBox, Rectangle2D bounds) {
        for (RowIterator iter = childrenIterator(parent); iter.hasNext(); ) {
            ...
            Item item = pickTop(row, hitBox, bounds);
            if (item != null)
                return item;
        }
        return null;
