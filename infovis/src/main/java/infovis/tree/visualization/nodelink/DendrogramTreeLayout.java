    ...
    public Dimension getPreferredSize(Visualization vis) {
        ...
        try {
            ...
            for (RowIterator iter = iterator(); iter.hasNext();) {
                ...
                if (rect != null) {
                    maxSize = Math.max(maxSize, rect.width);
            ...
            Dimension d;
            if (Orientation.isHorizontal(getOrientation())) {
                d = new Dimension((int) (depth * vs.getMaxSize()+maxSize), (int) len);
            }
            else {
                d = new Dimension((int) len, (int) (depth * vs.getMaxSize() + maxSize));
            }
