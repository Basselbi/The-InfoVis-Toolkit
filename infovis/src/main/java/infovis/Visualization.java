...
import infovis.utils.PropertyChange;
 ...
 */
public interface Visualization extends LabeledComponent, Orientable, PropertyChange {
...
//    static final String INVERSEPERMUTATION_COLUMN     = "#inversePermutation";
    ...
    /**
     * @return <code>true</code> if the visualization is invalidated
     */
     ...
     * or <code>null</code>.
     */
    void setVisualColumn(String name, Column column);
     ...
     */
    LabeledItem createLabelItem(Item row);
     ...
     */
    Item pickTop(double x, double y, Rectangle2D bounds);
     ...
     * @return int the index of the item on top.
     */
    Item pickTop(Rectangle2D hitBox, Rectangle2D bounds);
     ...
     * @param hitBox the bounds where the top item is searched.
     * @param bounds the total bounds where the visualization is displayed.
     * @param pick an {@link ArrayList} that will contain each Items
