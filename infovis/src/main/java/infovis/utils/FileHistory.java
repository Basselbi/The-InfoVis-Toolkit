...
public class FileHistory {
    ...
    /**
     * Interface that must be implemented by a GUI applcation frame that wants
    ...
    public static interface IFileHistory {
        ...
        /**
         * Return the size of the main application frame. It is used to center
        ...
        /**
         * Return the main application frame. It is used to center the file
    ...
    public FileHistory(IFileHistory caller) {
        this.caller = caller;
        historySerFile = System.getProperty("user.home") + FILE_SEPARATOR_STR //$NON-NLS-1$
                + caller.getApplicationName() + "_FILE_HISTORY.cfg"; //$NON-NLS-1$
        String max_itemnames_str = System.getProperty("itemnames.max", "9"); //$NON-NLS-1$ //$NON-NLS-2$
        try {
    ...
    /***************************************************************************
     * Initialize itemname and pathname arraylists from historySerFile. * build
     * up the additional entries in the File menu. *
     **************************************************************************/
            ...
            try {
                ...
                for (int i = 0; i < itemnameCount; i++) {
                    ...
                    MenuItemWithFixedTooltip item = new MenuItemWithFixedTooltip(
                            (i + 1) + ": " + (String) itemnameHistory.get(i)); //$NON-NLS-1$
                ...
                log.error("Trouble reading file history entries:", e);
            }
        }
    }
    ...
    public void saveHistoryEntries() {
        try {
            ...
            log.error("Trouble saving file history entries: ", e);
        }
    }

    /***************************************************************************
     * Insert the last loaded pathname into the File menu if it is not * present
     * yet. Only max pathnames are shown (the max number can be * set in
     * Jmon.ini, default is 9). Every item starts with * "<i>: ", where <i> is
     * in the range [1..max]. * The loaded itemname will become item number 1 in
     * the list. *
     **************************************************************************/
    ...
    public void processList() {
        ...
        deleteB.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (indicesToDelete.length > 0) {
                    ...
                    ArrayList itemnames = new ArrayList(oldFileHistorySize
                            - indicesToDelete.length);
                    ArrayList pathnames = new ArrayList(oldFileHistorySize
                            - indicesToDelete.length);
        ...
        dialog.setLocation(
                parentSize.width / 2 - mySize.width / 2,
     ...
     * used for the * itemname because we do not want to make the JMenuItem *
     * entry too wide. *
     **************************************************************************/
    ...
    /***************************************************************************
     * Create a tooltip location directly over the menu item, * ie, left allign
     * the tooltip text in "overlay" technique. *
     **************************************************************************/
    private final class MenuItemWithFixedTooltip extends JMenuItem {
        ...
        public Point getToolTipLocation(MouseEvent e) {
            ...
            String prefix = itemnameHistory.size() <= 9 ? "8: " : "88: "; //$NON-NLS-1$ //$NON-NLS-2$
