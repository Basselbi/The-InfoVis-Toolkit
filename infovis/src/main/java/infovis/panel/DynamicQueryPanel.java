...
public class DynamicQueryPanel extends AbstractControlPanel 
    ...
    /**
     * Release all the resources used or referenced by this panel.
     */
    public void dispose() {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == hideFilteredButton) {
            ...
            Permutation perm = getPermutation();
            perm
                    .filter(new InverseRowFilter(getVisualization()
                            .getSelection()));
            getVisualization().setPermutation(perm);
        }
    ...
    public void createDefaultControl(Column c) {
        ...
        rs.setEnabled(true);
        // TODO sort alphabetically the controls
        add(rs);
    ...
    static class SelectVisualColumnAction extends AbstractAction {
         ...
         *            the visualization
         */
        public SelectVisualColumnAction(
                String visual,
                DynamicQuery dq,
                Visualization vis) {
            super(visual);
    ...
    public void addPopupActions(JPopupMenu popup, final DynamicQuery dq) {
        ...
        for (Iterator iter = getVisualization().getVisualColumnIterator(); iter
                .hasNext();) {
            String visual = (String) iter.next();
            VisualColumnDescriptor vc = getVisualization()
                    .getVisualColumnDescriptor(visual);
            SelectVisualColumnAction action = new SelectVisualColumnAction(
                    visual,
                    dq,
                    getVisualization());
        ...
        popup.add(new AbstractAction("Show Values") {
            public void actionPerformed(ActionEvent e) {
                ...
                if (c instanceof BasicColumn) {
                    ...
                    for (RowIterator iter = map.keyIterator(); iter.hasNext();) {
                        int k = iter.nextRow();
                        model.addElement(col.getValueAt(k) + " (" + map.get(k)
                                + "x)");
                    }
                    JList list = new JList(model);
                    JComponent pane = new JScrollPane(list);
//                    if (col instanceof NumberColumn) {
//                        NumberColumn num = (NumberColumn) col;
//                        HistogramVisualization vis = new HistogramVisualization(
//                                num);
//                        vis
//                                .setOrientation(HistogramVisualization.ORIENTATION_EAST);
//                        VisualizationInteractor inter = InteractorFactory
//                                .createVisualizationInteractor(vis);
//                        vis.setInteractor(inter);
...
//                    }
                    ...
                    JFrame frame = new JFrame("Value list of column '"
                            + col.getName() + "'");
    ...
    /**
     * Adds Dynamic Query controls for all columns in the table unless one
     * already exists.
     */
    public void addMissingControls() {
        Comparator comp = new Comparator() {
            public int compare(Object o1, Object o2) {
                return ((Column) o1).getName().compareToIgnoreCase(
                        ((Column) o2).getName());
            }
        };
        TreeSet knownColumn = new TreeSet(comp);
        knownColumn.addAll(columnDynamicQuery.keySet());

        for (int col = 0; col < table.getColumnCount(); col++) {
            Column c = table.getColumnAt(col);
            if (filter(c))
                continue;
            if (!knownColumn.contains(c)) {
                createDefaultControl(c);
            }
            knownColumn.remove(c);
        }
