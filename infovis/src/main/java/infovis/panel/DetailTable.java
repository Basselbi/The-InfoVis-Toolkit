...
public class DetailTable 
    ...
    protected IntArrayList          columns;
    protected transient MutableTableModelEvent TMP_Event = new MutableTableModelEvent(this);
     ...
     * 
     * @param table
     *            the specified <code>Table</code> to manage
     * @param selection
     *            the column containing the selected items to manage in the
     *            table
     */
    public DetailTable(Table table, AbstractBooleanColumn selection) {
        this.table = table;
        if (selection == null) {
    ...
    public static void sizeColumn(JTable table, int i) {
        ...
        for (int row = 0; row < table.getRowCount(); row++) {
            ...
            if (s != null) {
                width = Math.max(
                		width, 
                		SwingUtilities.computeStringWidth(fm, s) + 2);
            }
        }
    ...
    public static JScrollPane createDetailJTable(
            final Table table,
            AbstractBooleanColumn selection) {
