 ...
 */
public class MinMaxDynamicQuery extends DefaultDoubleBoundedRangeModel
    ...
    class MaxDynamicQuery extends AbstractDynamicQuery {
        ...
        public boolean isFiltered(int row) {
            ...
            double min = numberColumn.coerce(getValue());
            boolean ret = !(v >= min);
            return ret;
        }
    ...
    class MinDynamicQuery extends AbstractDynamicQuery {
        ...
        public boolean isFiltered(int row) {
            if (column.isValueUndefined(row))
                return false;
            double v = numberColumn.getDoubleAt(row);
            double max = numberColumn.coerce(getValue() + getExtent());
            boolean ret = !(v <= max);
            return ret;
        }
    ...
    /**
     * Creates a MinMaxDynamicQuery with a min and max column.
    ...
    /**
     * 
     * @return the mim dynamic query
     */
    public DynamicQuery getMinDQ() {
        return minDQ;
    }

    /**
     * 
     * @return the max dynamic query
     */
    public DynamicQuery getMaxDQ() {
        return maxDQ;
    }
