...
/**
 * <b>AbstractItemRenderer</b> is a base class for implementing ItemRenderers.
 * 
...
public abstract class AbstractItemRenderer implements ItemRenderer, Cloneable {
    ...
    /**
     * Finds an item rendered by name in a hierarchy of item renderers.
     * @param name the name
     * @param ir the top-level of the hierarchy
     * @return an item renderer with the specified name or null
     */
    ...
    /**
     * Finds an item rendered by class in a hierarchy of item renderers.
     * @param c the class
     * @param ir the top-level of the hierarchy
     * @return an item renderer with the specified class or null
     */
    public static ItemRenderer findWithClass(Class c, ItemRenderer ir) {
        if (ir == null) {
            return null;
        }
        if (c.isInstance(ir)) {
        ...
        for (int i = 0; i < ir.getRendererCount(); i++) {
            ItemRenderer ret = findWithClass(c, ir.getRenderer(i));
            if (ret != null) {
                return ret;
            }
        }
    ...
    /**
     * Finds an item rendered by name in a visualization.
     * @param name the name
    ...
    /**
     * Replace a named item renderer by another one
     * @param named the named item renderer
     * @param ir the one that should replace it
     * @return true if the renderer has been replaced
     */
...
//    }
     ...
     */
    public boolean insertIn(Class clas, ItemRenderer ir) {
        if (ir == null) {
            return false;
        }
        if (clas.isInstance(ir)) {
            if (ir.getRendererCount() == 0) {
                ir.addRenderer(this);
            }
            else {
                addRenderer(ir.getRenderer(0));
                ir.setRenderer(0, this);
            }
            return true;
        }
        for (int i = 0; i < ir.getRendererCount(); i++) {
            boolean ret = insertIn(clas, ir.getRenderer(i));
            if (ret)
                return ret;
        }
        return false;
    ...
    public void uninstall(Graphics2D graphics) {
        for (int i = getRendererCount(); --i >= 0; ) {
