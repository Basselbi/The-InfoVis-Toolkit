...
import infovis.column.Algorithms;
...
import infovis.visualization.render.AbstractItemRenderer;
import infovis.visualization.render.ItemRendererFactory;
import infovis.visualization.render.LayoutVisual;
 ...
 *     display-name="Default Visualization"
 ...
 *      value="false"
 *
 * description: A component that visualizes a data structure.
 * 
 * @author Jean-Daniel Fekete
 * @version $Revision: 1.89 $
 */

public class DefaultVisualization extends TableProxy implements Visualization {
    private static final long                    serialVersionUID = -8085675695602493696L;
...
//        }
     ...
     * {@inheritDoc}
     */
    public void setVisualColumn(String name, Column newC) {
        VisualColumnDescriptor vc = getVisualColumnDescriptor(name);
        if (vc == null) {
...
//            else
    ...
    public FilterColumn getFilter() {
        ...
        return vf == null ? null : (FilterColumn)vf.getColumn();
    }
     ...
     *     bound="true"
     *     hidden="false"
     *     shortDescription="Orientation of the Visualization"
     *
     * @javabean.attribute
     *    name="enumerationValues"
     *    value=" \"NORTH\", SwingConstants.NORTH, null,
     *            \"SOUTH\", SwingConstants.SOUTH, null,
     *            \"EAST\", SwingConstants.EAST, null,
     *            \"WEST\", SwingConstants.WEST, null "
     *    rtexpr="array"
     *
     * @beaninfo 
     *   preferred: true
     *       bound: true
     *        enum: NORTH               Orientation.ORIENTATION_NORTH
     *              SOUTH               Orientation.ORIENTATION_SOUTH
     *              EAST                Orientation.ORIENTATION_EAST
     *              WEST                Orientation.ORIENTATION_WEST
     * description: The visualization's main orientation
     */
...
//                shapes.setSize(table.getLastRow()+1);
    ...
    /**
     * Pick all the items in a lasso selection.
     *
     * @param lasso the shape where the top item is searched.
     * @param hitBox the bounds where the top item is searched.
     * @param bounds the total bounds where the visualization is displayed.
     * @param pick an IntArrayList that will contain each row of items
     *        intersecting the hitBox.
    ...
    public ArrayList pickAll(
        ...
        for (RowIterator iter = iterator(); iter.hasNext();) {
            int row = iter.nextRow();
            Shape s = getShapeAt(row);
            if(lasso.contains(s.getBounds2D())){
                pick.add(getItem(row));
            }
        }
        return pick;
...
//        else 
     ...
     * @param newV the new value
     */
    public void firePropertyChange(String property, double oldV, double newV) {
        if (changeSupport == null) return;
        changeSupport.firePropertyChange(property, new Double(oldV), new Double(newV));
    }
    ...
    /**
     * Sets the ruler table.
     * @param t the table
     */
    public void setRulerTable(RulerTable t) {
        if (rulers == t) return;
        RulerTable old = rulers;
        rulers = t;
        firePropertyChange(PROPERTY_RULERS, old, t);
    }
    
    /**
     * Clears the ruler table.
     */
