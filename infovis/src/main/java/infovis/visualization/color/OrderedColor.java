...
/**
 * <b>OrderedColor</b> is the basic ColorVisualization for ordered values.
 * 
...
public class OrderedColor extends ColorVisualization {
    ...
    /**
     * Creates an OrderedColor for a number color and a color ramp.
     * @param column the number column
     * @param ramp the color ramp
     */
    ...
    public Color getColorForParameter(double t) {
        int i = (int)(t * cacheSize);
        if (i >= cacheSize) {
            i = cacheSize-1;
        }
        else if (i < 0) {
    ...
    public boolean isSameRamp(Color[] other) {
        if (ramp.length != other.length)
            return false;
        for (int i = 0; i < other.length; i++) {
            if (! ramp[i].equals(other[i]))
                return false;
        }
        return true;
    }
