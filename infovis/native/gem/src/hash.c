...
/* algorithm djb2 */
unsigned int strhash(const void *o)
{
    const char * str = (const char *)o;
    int c;
    int hash = 5381;
    while ((c = *str++))
        hash = hash * 33 + c;
...
unsigned int idhash(const void * o)
{
    return (unsigned int)o;
}


hash * hash_new(unsigned int capacity, hash_fn h_fn) {
    struct hash *h;
    int i, sind = -1;
