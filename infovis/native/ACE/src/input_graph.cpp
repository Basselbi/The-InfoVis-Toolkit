...
)
{
	...
	*nvtxs = read_int(fin, &end_flag);
    ...
    if (narcs < 0) {
      fprintf(stderr, "ERROR in graph file `%s':", inname);
      fprintf(stderr, " Invalid number of expected edges (%d).\n", narcs);
	if (fpStatsFile != NULL) {
	    fprintf(fpStatsFile, "ERROR in graph file `%s':", inname);
	...
	*eweights = NULL;
	...
	if (vtxnums) {
		...
		if (vertex == *nvtxs)
		    break;
		fprintf(stderr, "ERROR in graph file `%s':", inname);
		fprintf(stderr, " no vertex number in line %d.\n", line_num);
		if (fpStatsFile != NULL) {
		    fprintf(fpStatsFile, "ERROR in graph file `%s':", inname);
	...
	if (using_vwgts && new_vertex) {
	    ...
	    if (end_flag) {
	      fprintf(stderr, "ERROR in graph file `%s':", inname);
	      fprintf(stderr, " no weight for vertex %d.\n", vertex);
		if (fpStatsFile != NULL) {
		    fprintf(fpStatsFile, "ERROR in graph file `%s':", inname);
	    ...
	    if (weight <= 0) {
	      fprintf(stderr, "ERROR in graph file `%s':", inname);
	      fprintf(stderr, " zero or negative weight entered for vertex %d.\n", vertex);
		if (fpStatsFile != NULL) {
		    fprintf(fpStatsFile, "ERROR in graph file `%s':", inname);
		    fprintf(fpStatsFile,
	...
	while (!end_flag) {
	    ...
	    if (neighbor > *nvtxs) {
	      fprintf(stderr, "ERROR in graph file `%s':", inname);
	      fprintf(stderr, " nvtxs=%d, but edge (%d,%d) was input.\n",
	    ...
	    if (neighbor <= 0) {
	      fprintf(stderr, "ERROR in graph file `%s':", inname);
	      fprintf(stderr, " zero or negative vertex in edge (%d,%d).\n",
		       vertex, neighbor);
		if (fpStatsFile != NULL) {
		    fprintf(fpStatsFile, "ERROR in graph file `%s':", inname);
		    fprintf(fpStatsFile,
		...
		if (!self_edge && CHECK_INPUT) {
		  fprintf(stderr, "WARNING: Self edge (%d, %d) being ignored.\n",
			   vertex, vertex);
    		    if (fpStatsFile != NULL) {
		        fprintf(fpStatsFile,
		...
		if (found_flag) {
		  fprintf(stderr, "WARNING: Multiple occurences of edge (%d,%d) ignored\n",
			vertex, neighbor);
		    if (fpStatsFile != NULL) {
			fprintf(fpStatsFile,
		...
		if (end_flag) {
		  fprintf(stderr, "ERROR in graph file `%s':", inname);
		  fprintf(stderr, " no weight for edge (%d,%d).\n", vertex, neighbor);
		    if (fpStatsFile != NULL) {
		        fprintf(fpStatsFile, "ERROR in graph file `%s':", inname);
		        fprintf(fpStatsFile,
		...
		if (eweight <= 0 && CHECK_INPUT) {
		  fprintf(stderr, "WARNING: Bad weight entered for edge (%d,%d).  Edge ignored.\n",
			   vertex, neighbor);
    		    if (fpStatsFile != NULL) {
		        fprintf(fpStatsFile,
		...
		if (++nedges > 2*narcs) {
		  fprintf(stderr, "ERROR in graph file `%s':", inname);
		  fprintf(stderr, " at least %d adjacencies entered, but nedges = %d\n",
			nedges, narcs);
		    if (fpStatsFile != NULL) {
		        fprintf(fpStatsFile, "ERROR in graph file `%s':", inname);
		        fprintf(fpStatsFile,
    ...
    if (flag && CHECK_INPUT) {
      fprintf(stderr, "WARNING: Possible error in graph file `%s'\n", inname);
      fprintf(stderr, "         Data found after expected end of file\n");
        if (fpStatsFile != NULL) {
	    fprintf(fpStatsFile,
    ...
    if (self_edge > 1 && CHECK_INPUT) {
      fprintf(stderr, "WARNING: %d self edges were read and ignored.\n", self_edge);
        if (fpStatsFile != NULL) {
	    fprintf(fpStatsFile,
	...
	/* Make sure narcs was reasonable. */
		if (nedges + 2 * self_edge != 2 * narcs &&
			...
			fprintf(stderr, "WARNING: I expected %d edges entered twice, but I only count %d.\n", narcs, nedges);
    		if (fpStatsFile != NULL) 
				fprintf(fpStatsFile,
